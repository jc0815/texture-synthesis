...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
    zeros_TODOMask = np.argwhere(TODOMask == 0) # get all the zeros in TODOMask
    ...
    for r in range(ssd_rows):
        for c in range(ssd_cols):
            ...
            left_x, top_y = r, c # get top left corner of patch
            right_x, bottom_y = (left_x + (2 * patchL), top_y + (2 * patchL)) # get bottom right corner of patch
            textureIm_patch = textureIm[left_x:right_x+1, top_y: bottom_y+1] # textureIm_patch within the border
            
            TODO_X = zeros_TODOMask[:, 0] # for all x = 0 in TOODMask
            TODO_y = zeros_TODOMask[:, 1] # for all y = 0 in TOODMask
            
            R = TODOPatch[TODO_X, TODO_y, 0] - textureIm_patch[TODO_X, TODO_y, 0] # RED - difference with patch and textureIm_patch
            G = TODOPatch[TODO_X, TODO_y, 1] - textureIm_patch[TODO_X, TODO_y, 1] # GREEN - difference with patch and textureIm_patch
            B = TODOPatch[TODO_X, TODO_y, 2] - textureIm_patch[TODO_X, TODO_y, 2] # BLUE - difference with patch and textureIm_patch
            
            SSD[r][c] = np.sum(R**2 + G**2 + B**2) # sum of squared difference for each value
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
    ...
    for i in range(patchSize):
        for j in range(patchSize):
            ...
            # The patch is centered on iPatchCenter, jPatchCenter in the image imHole
            
            current_hole_x = iPatchCenter - patchL + i # get current hole x
            current_hole_y = jPatchCenter - patchL + j # get current hole y
            current_patch_x = iMatchCenter - patchL + i # get patch x to replace hole x
            current_patch_y = jMatchCenter - patchL + j # get patch y to replace hole y
            
            if TODOMask[i][j] == 1:
                imHole[current_hole_x][current_hole_y] = textureIm[current_patch_x][current_patch_y] # replace image hole
            else:
                continue
...
#
try:
    imname = "./images/" + str(sys.argv[1]) + ".jpg"
    im = Image.open(imname).convert('RGB')
except:
    im = Image.open('./images/donkey.jpg').convert('RGB')
...
Image.fromarray(imHole).convert('RGB').save('./results/results.jpg')
